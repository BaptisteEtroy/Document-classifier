...
def train_medical_model(data_dir='app/data', output_model_path='app/models/medical_model.pkl'):
    """Train a model to identify medical documents"""
    ...
    other_files = []
    for category in ['invoices', 'contracts', 'others']:
        category_dir = os.path.join(data_dir, category)
        if os.path.exists(category_dir):
            other_files.extend(list_files_in_directory(category_dir))
    
    if not os.path.exists(medical_dir):
        print(f"Medical directory {medical_dir} not found. Creating it...")
        os.makedirs(medical_dir, exist_ok=True)
        print(f"Please add medical documents to {medical_dir} and run again.")
        return
    
    medical_files = list_files_in_directory(medical_dir)
    
    if not medical_files:
        print(f"No medical files found in {medical_dir}. Please add some and run again.")
        return
    
    if not other_files:
        print("No non-medical files found for negative examples. Using only medical features.")
    
    # Create dataset
    X = []
    y = []
    
    # Process medical files (positive examples)
    ...
    # Process non-medical files (negative examples)
    ...
    print(f"Medical model accuracy: {accuracy:.4f}")
    ...
    print(f"Medical model saved to {output_model_path}")
